 ...
 * @param {string} query
 * @param {{ limit?: number, color?: string }} [options]
...
export async function searchIcons(query, options = {}) {
  ...
  const icons = (data.icons || []).map((id) => {
    ...
    return {
      ...
      prefix,
      name,
      svgUrl: iconifySvgUrl(prefix, name, color),
    };
  });
...
/**
 * Build the Iconify CDN URL for an SVG icon.
 * @param {string} prefix
 * @param {string} name
 * @param {string} [color] CSS color (e.g. '#ffffff') applied to monochrome icons
 */
export function iconifySvgUrl(prefix, name, color = '#ffffff') {
  return `${ICONIFY_SVG}/${prefix}/${name}.svg?height=128&color=${encodeURIComponent(color)}`;
}
 ...
 */
export async function resolveIconifyToDataUrl(iconId, color = '#ffffff') {
  const [prefix, name] = String(iconId).split(':');
  if (!prefix || !name) {
    throw new Error('Invalid icon id');
  }

  const svgUrl = iconifySvgUrl(prefix, name, color);
